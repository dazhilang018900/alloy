...
void svd2(
    ...
    v12 = -v21;
    v22 = v11;
    u11 = (s1 != 0) ? (a11 * v11 + a12 * v21) / s1 : 1;
    u21 = (s1 != 0) ? (a21 * v11 + a22 * v21) / s1 : 0;
    u12 = (s2 != 0) ? (a11 * v12 + a12 * v22) / s2 : -u21;
    u22 = (s2 != 0) ? (a21 * v12 + a22 * v22) / s2 : u11;
}
...
inline void multAB(float a11, float a12, float a13, float a21, float a22, float a23,
...
inline void multAtB(float a11, float a12, float a13, float a21, float a22, float a23,
		float a31, float a32, float a33,
		//
		float b11, float b12, float b13, float b21, float b22, float b23,
		float b31, float b32, float b33,
...
void quatToMat3(const float * qV, float &m11, float &m12, float &m13,
		float &m21, float &m22, float &m23, float &m31, float &m32,
		float &m33) {
	float w = qV[3];
	float x = qV[0];
	float y = qV[1];
...
void approximateGivensQuaternion(float a11, float a12, float a22, float &ch,
		float &sh) {
	/*
...
void jacobiConjugation(const int x, const int y, const int z, float &s11,
...
void QRGivensQuaternion(float a1, float a2, float &ch, float &sh) {
	...
	float rho = std::sqrt(a1 * a1 + a2 * a2);

	sh = rho > epsilon ? a2 : 0;
	ch = std::abs(a1) + fmax(rho, epsilon);
	bool b = a1 < 0;
	condSwap(b, sh, ch);
...
void svd3(
	...
	QRDecomposition(b11, b12, b13, b21, b22, b23, b31, b32, b33, u11, u12, u13,
			u21, u22, u23, u31, u32, u33, s11, s12, s13, s21, s22, s23, s31,
			s32, s33);
}
}
