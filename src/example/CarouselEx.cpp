...
bool CarouselEx::init(Composite& rootNode) {
	...
	for (int n = 0; n < N; n++) {
		DrawPtr region = DrawPtr(
				new Draw(MakeString() << "[" << (char) ('A' + n) << "]",
						CoordPX(0.0f, 0.0f),
						CoordPerPX(0.0f, 1.0f, 200.0f-Composite::scrollBarSize, 0.0f)));
		region->onDraw =
				[=,n](AlloyContext* context,const box2px& bounds) {
					NVGcontext* nvg = context->nvgContext;
					nvgBeginPath(nvg);
					nvgRoundedRect(nvg, bounds.position.x + 2, bounds.position.y + 2,
							bounds.dimensions.x - 4, bounds.dimensions.y - 4-Composite::scrollBarSize,context->theme.CORNER_RADIUS);
	...
	rootNode.backgroundColor = MakeColor(getContext()->theme.DARKER);
	hvscroller->borderWidth = UnitPX(1.0f);
	hvscroller->borderColor = MakeColor(getContext()->theme.LIGHTER);
		...
		for (int m = 0; m < binSizes[n]; m++) {
			DrawPtr region = DrawPtr(
					new Draw(MakeString() << "[" << (char) ('A' + n) <<":"<<m<< "]",
							CoordPX(0.0f, 0.0f),
							CoordPerPX(1.0f, 0.0f, 0.0f, 100.0f-Composite::scrollBarSize)));
			region->onDraw =
					[=,n,m](AlloyContext* context,const box2px& bounds) {
						NVGcontext* nvg = context->nvgContext;
						nvgBeginPath(nvg);
						nvgRoundedRect(nvg, bounds.position.x + 2, bounds.position.y + 2,bounds.dimensions.x - 4-Composite::scrollBarSize, bounds.dimensions.y - 4,context->theme.CORNER_RADIUS);
						nvgStrokeWidth(nvg, 2.0f);
